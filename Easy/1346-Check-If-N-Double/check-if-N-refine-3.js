...
var checkIfExist = function (arr) {
    ...
    let lastNeg = negNumbers[negNumbers.length - 1];
    for (let i = 0; i < negNumbers.length - 1; i++) {
        let currentNumber = negNumbers[i];
        let currentNumberTimesTwo = currentNumber * 2;
        if (currentNumberTimesTwo > lastNeg) {
            break;
        }
        else {
    ...
    let lastZero = zeroNumbers[zeroNumbers.length - 1];
    for (let i = 0; i < zeroNumbers.length - 1; i++) {
        let currentNumber = zeroNumbers[i];
        let currentNumberTimesTwo = currentNumber * 2;
        if (currentNumberTimesTwo > lastZero) {
            break;
        }
        else {
