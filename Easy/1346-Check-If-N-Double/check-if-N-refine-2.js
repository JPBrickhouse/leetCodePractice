...
// Attempt to cut down on brute force
...
var checkIfExist = function (arr) {
    ...
    function miniCheck(sortedArray) {
        let lastEntry = sortedArray[sortedArray.length - 1];
        for (let i = 0; i < sortedArray.length - 1; i++) {
            let currentNumber = sortedArray[i];
            let currentNumberTimesTwo = currentNumber * 2;
            if (currentNumberTimesTwo > lastEntry) {
                break;
            }
            else {
                for (let j = i + 1; j < sortedArray.length; j++) {
                    let comparisonNumber = sortedArray[j]
    ...
    // Then running it three times
    const ans1 = miniCheck(posNumbers)
    const ans2 = miniCheck(negNumbers)
    const ans3 = miniCheck(zeroNumbers)

    // Checking if anything is true, otherwise, returning false
    if (ans1 || ans2 || ans3) {
        return true;
    }
    else {
